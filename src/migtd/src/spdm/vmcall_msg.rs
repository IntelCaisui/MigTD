use spdmlib::{common::SpdmTransportEncap, error::{SPDM_STATUS_DECAP_FAIL, SPDM_STATUS_ENCAP_FAIL}};
...
pub const VMCALL_SPDM_VERSION: [u8; 2] = [1, 0];
...
impl Codec for VmCallMessageHeader {
    ...
    fn read(r: &mut Reader) -> Option<Self> {
        ...
        if signature != VMCALL_SPDM_SIGNATURE { // Verify 'SPDM' signature
            return None;
        }
        let version = <[u8; 2]>::read(r)?;
...
impl SpdmTransportEncap for VmCallTransportEncap {
    async fn encap(
        ...
        vmcall_msg_header.encode(&mut writer).map_err(|_|SPDM_STATUS_ENCAP_FAIL)?;
        let header_size = writer.used();

        if transport_buffer.len() < header_size + spdm_buffer.len() {
            return Err(SPDM_STATUS_ENCAP_FAIL);
        }
        transport_buffer[header_size..(header_size + spdm_buffer.len())].copy_from_slice(&spdm_buffer);
    ...
    async fn decap(
        ...
        let vmcall_msg_header = VmCallMessageHeader::read(&mut reader).ok_or(SPDM_STATUS_DECAP_FAIL)?;
        ...
        let secured_message = if vmcall_msg_header.msg_type == VmCallMessageType::SecuredSpdmMessage {
            true
        } else {
            false
        };

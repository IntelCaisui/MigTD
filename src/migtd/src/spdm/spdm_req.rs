...
use crate::{
    ...
    spdm::{vmcall_msg::VmCallTransportEncap, *},
};
...
pub async fn spdm_requester_transfer_msk(
    ...
    let res = with_timeout(
        SPDM_TIMEOUT,
        send_and_receive_sdm_exchange_migration_info(
            spdm_requester,
            exchange_info,
            remote_info,
            Some(session_id),
...
async fn send_and_receive_pub_key(spdm_requester: &mut RequesterContext) -> SpdmResult {
    ...
    let pub_key = signing_key.public_key_spki();
    ...
    let pub_key_element = VdmMessageElement {
        ...
        length: pub_key.len() as u16, // to be updated
    };
    cnt += pub_key_element
        .encode(&mut writer)
        .map_err(|_| SPDM_STATUS_BUFFER_FULL)?;
    cnt += writer.extend_from_slice(pub_key.as_slice()).unwrap();
    ...
    let pub_key_peer = reader.take(vdm_element.length as usize).unwrap();

    let mut my_pub_key = SpdmCertChainData {
        data_size: pub_key.len() as u32,
        data: [0u8; config::MAX_SPDM_CERT_CHAIN_DATA_SIZE],
    };
    my_pub_key.data[..pub_key.len()].copy_from_slice(&pub_key);
    spdm_requester.common.provision_info.my_pub_key = Some(my_pub_key);

    let mut peer_pub_key = SpdmCertChainData {
        data_size: pub_key_peer.len() as u32,
        data: [0u8; config::MAX_SPDM_CERT_CHAIN_DATA_SIZE],
    };
    peer_pub_key.data[..pub_key_peer.len()].copy_from_slice(pub_key_peer);
    spdm_requester.common.provision_info.peer_pub_key = Some(peer_pub_key);
...
async fn send_and_receive_sdm_exchange_migration_info(
    ...
    remote_info.min_ver = u16::read(reader).unwrap();
    remote_info.max_ver = u16::read(reader).unwrap();
    ...
    remote_info.key.fields = <[u64; 4]>::read(reader).unwrap();

...
async fn send_and_receive_pub_key(spdm_requester: &mut RequesterContext) -> SpdmResult {
    ...
    let response = spdm_requester.send_spdm_vendor_defined_request_ex(
        None,
        &send_buffer[..used],
        &mut receive_buffer,
    ).await?;
    ...
    let _response_header = SpdmMessageHeader::read(&mut reader).ok_or(SPDM_STATUS_INVALID_MSG_SIZE)?;
    let response_payload = SpdmVdmResponsePayload::spdm_read(&mut spdm_requester.common, &mut reader)
        .ok_or(SPDM_STATUS_INVALID_MSG_SIZE)?;

    let mut reader = Reader::init(
        &response_payload.rsp_payload[..response_payload.rsp_length as usize],
    );
    ...
    let vdm_pub_key_dst_hash =
        digest_sha384(&response).map_err(|_| SPDM_STATUS_CRYPTO_ERROR)?;

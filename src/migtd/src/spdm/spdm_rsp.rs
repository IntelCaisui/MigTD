// Spdm Responder is the MigTD dst side implementation. 11
...
const ECDSA_P384_SHA384_PUBLIC_KEY_LENGTH: usize = 0x78;
pub static SIGNING_KEY: Mutex<[u8; ECDSA_P384_SHA384_PRIVATE_KEY_LENGTH]> =
    Mutex::new([0u8; ECDSA_P384_SHA384_PRIVATE_KEY_LENGTH]);

const EXCHANGE_INFO_BUFFER_SIZE: usize = 36;
pub static EXCHANGE_INFO_LOCAL: Mutex<[u8; 36]> = Mutex::new([0u8; EXCHANGE_INFO_BUFFER_SIZE]);
pub static EXCHANGE_INFO_PEER: Mutex<[u8; 36]> = Mutex::new([0u8; EXCHANGE_INFO_BUFFER_SIZE]);
...
pub async fn spdm_responder_transfer_msk(
    ...
    remote_info: &mut ExchangeInformation,
) -> Result<(), SpdmStatus> {
    {
        let mut exchange_info_local = EXCHANGE_INFO_LOCAL.lock();
        let len = exchange_info_local.len();
        let mut writer = Writer::init(&mut exchange_info_local[..len]);
        exchange_info
            .min_ver
            .encode(&mut writer)
    ...
    let _res = with_timeout(SPDM_TIMEOUT, rsp_handle_message(spdm_responder)).await;

    {
        let mut exchange_info_peer = EXCHANGE_INFO_PEER.lock();
        let len = exchange_info_peer.len();
        let mut reader = Reader::init(&exchange_info_peer[..len]);
        remote_info.min_ver = u16::read(&mut reader).unwrap();
        remote_info.max_ver = u16::read(&mut reader).unwrap();
        remote_info.key.fields = <[u64; 4]>::read(&mut reader).unwrap();
        exchange_info_peer[..len].zeroize();
    }
...
pub fn handle_exchange_pub_key_req(
    ...
    let peer_pub_key = VdmMessageElement::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    if peer_pub_key.element_type != VdmMessageElementType::PubKeyMy
        || vdm_request.element_count != 1
    {
        return Err(SPDM_STATUS_INVALID_MSG_FIELD);
    }
    let peer_pub_key_length = peer_pub_key.length as usize;
    let mut pub_key_buf = [0u8; ECDSA_P384_SHA384_PUBLIC_KEY_LENGTH];
    for d in pub_key_buf.iter_mut().take(peer_pub_key_length) {
        *d = u8::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    }

    let mut peer_pub_key_prov = SpdmCertChainData {
        data_size: peer_pub_key_length as u32,
        data: [0u8; config::MAX_SPDM_CERT_CHAIN_DATA_SIZE],
    };
    peer_pub_key_prov.data[..peer_pub_key_length]
        .copy_from_slice(&pub_key_buf[..peer_pub_key_length]);
    ...
    // Save my private key to SIGNING_KEY for signing
    let mut key_bytes = SIGNING_KEY.lock();
    let private_key = signing_key.private_key();
    key_bytes[..private_key.len()].copy_from_slice(private_key);
...
pub fn handle_exchange_mig_attest_info_req(
    ...
    if vdm_request.element_count != 3 {
        error!(
    ...
    let mig_policy_hash = digest_sha384(mig_policy.as_bytes()).unwrap();
...
pub fn handle_exchange_mig_info_req(
    ...
    // Open: How to make sure attestion is done before mig info?
    if session_id.is_none() {
        return Err(SPDM_STATUS_INVALID_STATE_LOCAL);
    }
    let session = responder_context
    ...
    let mut exchange_info_peer = EXCHANGE_INFO_PEER.lock();
    let len = exchange_info_peer.len();
    let mut writer = Writer::init(&mut exchange_info_peer[..len]);

    let mig_export_version =
    ...
    if mig_export_version.element_type != VdmMessageElementType::MigrationExportVersion
        || mig_export_version.length != 4
        || vdm_request.element_count != 2
    {
        error!("invalid migration info payload!\n");
        return Err(SPDM_STATUS_INVALID_MSG_FIELD);
    }
    let mig_session_key = VdmMessageElement::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    if mig_session_key.element_type != VdmMessageElementType::ForwardMigrationSessionKey
        || mig_session_key.length != 32
    {
        error!("invalid forward migration session key!\n");
        return Err(SPDM_STATUS_INVALID_MSG_FIELD);
    }
    let mut mig_session_key_buf = [0u8; 32];
    for d in mig_session_key_buf.iter_mut() {
        *d = u8::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    }

    min_export_version.encode(&mut writer).unwrap();
    max_export_version.encode(&mut writer).unwrap();
    writer.extend_from_slice(&mig_session_key_buf).unwrap();

    let mut exchange_info_local = EXCHANGE_INFO_LOCAL.lock();
    let len = exchange_info_local.len();
    let reader = &mut Reader::init(&exchange_info_local[..len]);
    let min_import_version = u16::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    let max_import_version = u16::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    let mut mig_session_key_buf = [0u8; 32];
    for d in mig_session_key_buf.iter_mut() {
        *d = u8::read(reader).ok_or(SPDM_STATUS_INVALID_MSG_FIELD)?;
    }
    exchange_info_local[..len].zeroize();
    ...
    cnt += mig_session_key_element
        .encode(&mut writer)
        .map_err(|_| SPDM_STATUS_BUFFER_FULL)?;
    cnt += writer.extend_from_slice(&mig_session_key_buf).unwrap();
...
fn sign_ecdsa_asym_algo(
    ...
    let key_bytes = SIGNING_KEY.lock();
